...
命令行：
    python workspace__infra.py list
    python workspace__infra.py register-legacy --name 马山三标段
    python workspace__infra.py create --name 样例仓库 --slug sample
    python workspace__infra.py switch sample
    python workspace__infra.py info [slug]
    python workspace__infra.py reset <slug> --confirm     # 可回滚：旧库改名保留
    python workspace__infra.py delete <slug> --confirm    # 可回滚：数据目录移入 _bak
"""
...
_PATH_ATTRS: tuple[tuple[str, str, str], ...] = (
    ("hub_pipeline__desens", "HUB_DIR", "hub"),
    ("l1_extract__summary_hash", "HUB_DIR", "hub"),
    ("doc_index__graph", "HUB_DIR", "hub"),
    ("graph_query__graph_walk", "HUB_DIR", "hub"),
    ("entity_repair__desens", "HUB_DIR", "hub"),
    ("doc_lifecycle__desens", "HUB_DIR", "hub"),
    ("doc_lifecycle__desens", "OUTPUT_DIR", "output"),
    ("scanner_core__scan", "INPUT_DIR", "input"),
    ("scanner_core__scan", "OUTPUT_DIR", "output"),
    # hub_pipeline 在导入期把扫描缓存目录 **复制** 成了自己的常量，热切换要一起改
    ("hub_pipeline__desens", "CACHE_DIR", "output"),
    # 对话记录：历史问答里带着本仓库的证据，不能跨仓库看到
    ("chat_store__ui", "CHAT_ROOT", "chat"),
    ("chat_store__ui", "RECYCLE_ROOT", "chat_recycle"),
)
...
def apply_active(entry: dict | None = None, *, persist: bool = False) -> dict:
    ...
    **不建库、不写数据**，只切"进程往哪读、往哪写"。
    ...
    mod = sys.modules.get("database_serv__infra")
    if mod is not None:
...
def ensure_db(dbname: str, *, reset: bool = False) -> str:
    ...
    from database_serv__infra import ADMIN_DB_CONFIG, render_init_sql
...
def _create_first_user(username: str, password: str, email: str, phone: str) -> tuple[bool, str]:
    ...
    from database_serv__infra import register_user
...
def _rename_db(old: str, new: str) -> str:
    ...
    from database_serv__infra import ADMIN_DB_CONFIG
...
def code_permission_points() -> set[str]:
    ...
    try:
        from database_serv__infra import DECRYPT_PERMISSION_BY_CATEGORY

        pts |= set(DECRYPT_PERMISSION_BY_CATEGORY.values())
    except Exception:
        pass
    for mod_name, attr in (("project_registry__desens", "DECRYPT_PERMISSION"),
                           ("self_entity__desens", "DECRYPT_PERMISSION")):
        try:
...
def main(argv: list[str] | None = None) -> int:
    ...
    if cmd == "list":
        ...
        if not reg["workspaces"]:
            print("还没有登记任何仓库：先跑 `python workspace__infra.py register-legacy`")
            return 1
