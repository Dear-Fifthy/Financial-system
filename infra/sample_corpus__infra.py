...
用法：
    python -m infra.sample_corpus__infra --list
    python -m infra.sample_corpus__infra --create --load            # 建样例库 + 装载
    python -m infra.sample_corpus__infra --create --reset --load    # 重建（丢样例库旧数据）
    python -m infra.sample_corpus__infra --verify                   # 真值自检 + 孤儿编号检查
"""
...
def _hub_dir() -> Path:
    from desens.ai_guard__desens import hub_dir  # 惰性导入：确保 APP_DB_NAME 已生效
...
def ensure_db(dbname: str, *, reset: bool = False) -> str:
    ...
    """
    from infra.workspace__infra import ensure_db as _ws_ensure_db
...
def register_mappings(legend_data: dict | None = None) -> dict:
    ...
    """
    from infra.database_serv__infra import SECRET_CATEGORIES, MappingDbStore
...
def hashes_for(features: dict) -> dict[str, str]:
    ...
    （线上特征值也是编号），不是本装载器的简化。
    """
    from ai.ai_parser__ai import _date_parts, hash_feature_value  # noqa: PLC0415
...
def load_docs(docs: list[tuple[Path, dict]] | None = None) -> dict:
    from infra.database_serv__infra import get_admin_connection
    from l1.l1_extract__summary_hash import EXTRACT_VERSION, project_hub_file
...
def verify() -> dict:
    ...
    """
    from infra.database_serv__infra import ENTITY_TABLES, get_admin_connection
...
def cmd_smoke() -> int:
    ...
    import graph.graph_query__graph_walk as gw
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.workspace:
        ...
        if not entry:
            print(f"❌ 没有这个仓库：{args.workspace}（先看 python -m infra.workspace__infra list）")
            return 2
    ...
    prod_dbs = {PROD_DB}
    try:
        import infra.workspace__infra as _ws2
    ...
    if target_db in prod_dbs and not args.allow_prod:
        print(f"❌ 拒绝把样例写进真实语料库 {target_db}（这是保护：会混进真语料与真编号）。\n"
              ...
              f"   python -m infra.workspace__infra create --name 样例仓库 --slug sample\n"
              f"   python -m infra.sample_corpus__infra --workspace sample --create --load")
        return 2
