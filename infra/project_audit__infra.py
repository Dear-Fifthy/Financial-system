...
这些要么该清、要么绝不能入库，靠人记容易漏，所以做成一条命令：
    ...
    python -m infra.project_audit__infra            # 人看的报告
    python -m infra.project_audit__infra --json     # 机器读（CI/脚本用）
...
# 层包：模块按层搬进文件夹后，"顶层模块"= 这些目录下的 .py（文件名仍带 `__层` 后缀）
LAYER_DIRS = ("l1", "scan", "desens", "graph", "rag", "ai", "ui", "infra")
# 文件名后缀与所在层**故意**不一致的几个（历史命名，搬动时保持模块名不变）
SUFFIX_EXCEPTIONS = {
    "doc_index__graph": "graph",
    "edge_build__graph_edges": "graph",
    "graph_query__graph_walk": "graph",
    "l1_extract__summary_hash": "l1",
    "l1_facts__fact_list": "l1",
}


def layer_modules() -> dict[str, Path]:
    """模块文件名 -> 路径（键仍用文件名，日志读起来和搬动前一样）。"""
    out: dict[str, Path] = {}
...
def tracked_files() -> list[Path]:
    ...
    try:
        from infra import proc__infra as _proc     # 静默子进程：别弹黑窗口

        out = _proc.run(["git", "ls-files"], cwd=ROOT, capture_output=True,
...
def module_references(exclude_prefixes: tuple[str, ...] = ()) -> dict[str, list[str]]:
    """模块 -> 提到过它的文件（用来找"谁都不用的模块"）。扫的是各层包，不再是顶层。"""
    mods = layer_modules()
    scan: list[Path] = []
    for pat in ("*.md", "*.bat", "*.sql"):
        scan.extend(ROOT.glob(pat))
    for layer in LAYER_DIRS:
        scan.extend((ROOT / layer).glob("*.py"))
    for sub in ("sample_docs", "docs"):
        d = ROOT / sub
        if d.exists():
            scan.extend(d.rglob("*"))
    texts = {}
    ...
    for fname, path in mods.items():
        if fname.startswith(exclude_prefixes):
            continue
        stem = path.stem
        hits = [p.name for p, t in texts.items() if p != path and stem in t]
        out[fname] = sorted(set(hits))
    return out
...
def report() -> dict:
    ...
    for m, hits in refs.items():
        if hits:
            continue
        text = paths[m].read_text(encoding="utf-8", errors="ignore")
        if '__name__ == "__main__"' in text:
            continue
        orphans.append(m)
    orphans.sort()

    # 结构检查①：根目录**不该**再有模块 .py（模块都在 <层>/ 下，根上只留入口/配置/文档）
    root_stray_py = sorted(p.name for p in ROOT.glob("*.py"))
    # 结构检查②：层包里的模块名后缀要与所在层一致（少数历史命名在例外表里）
    mislayered: list[str] = []
    for layer in LAYER_DIRS:
        for p in sorted((ROOT / layer).glob("*.py")):
            if p.name == "__init__.py":
                continue
            want = SUFFIX_EXCEPTIONS.get(p.stem)
            ok = (want == layer) if want else p.stem.endswith("__" + layer)
            if not ok:
                mislayered.append(f"{layer}/{p.name}")
    ...
    wrongly_ignored = [x for x in ("init_db.sql", "ui/ui_kit__ui.py", "infra/sample_corpus__infra.py")
    ...
    for d in sorted(p for p in ROOT.iterdir() if p.is_dir() and p.name not in SKIP_DIRS):
        n = sum(1 for f in d.rglob("*") if f.is_file() and "__pycache__" not in f.parts)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.json:
        ...
        return 1 if (res["empty_files"] or res["temp_candidates"]
                     or res["gitignore"]["missing"] or res["secret_leaks"]
                     or res["root_stray_py"] or res["mislayered"]) else 0
    ...
    print("   （注：入口脚本如 scan/scanner_entrance__scan.py 本来就没人 import，属正常）")
    print(f"\n⑤ 结构：根目录多余的 .py {res['root_stray_py'] or '（无，模块都在 <层>/ 下）'}")
    print(f"   层包里后缀与所在层不符的模块：{res['mislayered'] or '（无）'}")
    ...
    bad = bool(res["empty_files"] or res["temp_candidates"] or res["gitignore"]["missing"]
               or res["secret_leaks"] or res["root_stray_py"] or res["mislayered"])
