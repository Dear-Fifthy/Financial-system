...
# ---------- 表/列 ----------
def _tables() -> dict:
    from infra.database_serv__infra import ENTITY_TABLES
...
def list_entries(category: str, *, limit: int = 2000) -> list[dict]:
    ...
    from infra.database_serv__infra import SECRET_CATEGORIES, get_connection
...
def scan_dirty(categories: tuple[str, ...] | None = None) -> list[dict]:
    ...
    out: list[dict] = []
    tables = _tables()
    from infra.database_serv__infra import SECRET_CATEGORIES
...
def register(category: str, real_value: str, *, user: dict | None = None,
    ...
    if parent_code and category in ("project", "company"):
        ...
        if category in SECRET_CATEGORIES:
            ...
            from infra.database_serv__infra import encrypt_value
