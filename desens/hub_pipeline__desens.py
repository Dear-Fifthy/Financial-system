...
from scan.scanner_core__scan import BASE_DIR, OUTPUT_DIR as CACHE_DIR, process_file
from infra.database_serv__infra import MappingDbStore
import desens.project_registry__desens as project_registry__desens   # 项目名称自定义加密（管理员登记 → 编号）
import desens.self_entity__desens as self_entity__desens        # 本公司（我方主体）：全局脱敏 + 编号特殊提示
...
from desens.contract_rules__desens import PARTY_A_RE, extract_contract_fields  # noqa: E402
...
def _build_mask_token_re() -> re.Pattern:
    ...
    """
    from infra.database_serv__infra import ENTITY_TABLES, SECRET_CATEGORIES, _CODE_PREFIX
...
def rescan_hub_classifications(    *,
    ...
    import desens.ledger_inbox__desens as ledger_inbox__desens
...
def process_file_to_hub(
    ...
    *,
    ...
    def on_page_json(page_path: Path) -> None:
        ...
        masked_tables: list[dict] = []
        try:
            from desens.ocr_tables__desens import collect_tables_from_page_json
    ...
    def consumer() -> None:
        ...
        try:
            while True:
                ...
                try:
                    import desens.offset_map__desens as _om
                ...
                try:
                    import desens.ocr_tables__desens as _ot
    ...
    if ai_active:
        try:
            from ai.ai_parser__ai import process_hub_file
    ...
    try:
        from l1.l1_extract__summary_hash import project_hub_file
