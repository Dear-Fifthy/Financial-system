...
def approve(item_id: int, user: dict | None = None, *, fields: dict | None = None,
    ...
    from database_serv__infra import api_save_contract_from_ai, api_save_invoice_from_ai
...
def hub_json_files() -> list[Path]:
    ...
    try:
        from ai_guard__desens import hub_dir
    ...
    for p, doc, doc_key in parsed:
        ...
        if kind == "contract" and gate.passed and store is not None:
            try:
                ...
                from contract_rules__desens import extract_contract_fields
