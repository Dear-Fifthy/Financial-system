...
from ui.ui_kit__ui import hint as kit_hint
...
from scan.scan_control__scan import ScanCancelled
...
class LedgerStatusDialog(QDialog):
    ...
    def _open_full(self) -> None:
        """跳到「台账全字段管理」（同一张台账）。"""
        from ui.ledger_admin__ui import LedgerAdminDialog
...
class ScanWorker(QObject):
    ...
    def run(self) -> None:
        try:
            from desens.hub_pipeline__desens import process_file_to_hub
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, current_user: dict) -> None:
        ...
        _ws_name = ""
        try:
            import infra.workspace__infra as _ws
    ...
    def _confirm_self_company(self) -> None:
        try:
            import desens.self_entity__desens as self_entity
            ...
            if not self_entity.needs_confirmation():
                ...
                for e in self_entity.list_entries(self.current_user):
                    if e.get("is_primary"):
                        ...
                        break
                return
            from ui.project_admin__ui import confirm_self_company
    ...
    def _add_paths(self, raw_paths: list[Path]) -> None:
        ...
        if self._fix_mode_check.isChecked():
            ...
            return
        try:
            from scan.scanner_core__scan import expand_inputs
        ...
        for source_root, file_path in inputs:
            ...
            try:
                from scan.ocr_priority__scan import needs_ocr
    ...
    def _open_output_directory(self) -> None:
        from desens.hub_pipeline__desens import HUB_DIR
    ...
    def _stop_scan(self) -> None:
        ...
        try:
            import scan.runtime_lane__scan as runtime_lane
    ...
    def _delete_selected_document(self) -> None:
        ...
        try:
            import desens.doc_lifecycle__desens as doc_lifecycle__desens
    ...
    def _open_project_registry(self) -> None:
        ...
        from ui.project_admin__ui import ProjectRegistryDialog  # 惰性导入，避免拖慢启动

        ProjectRegistryDialog(self, self.current_user).exec()

    def _open_hub_admin(self) -> None:
        """设置 → Hub 状态与文件格式化（仅 admin）：查看源文件、删指定文件、格式化。"""
        from ui.hub_admin__ui import HubAdminDialog  # 惰性导入，避免拖慢启动
    ...
    def _open_workspaces(self) -> None:
        ...
        """
        from ui.workspace_admin__ui import WorkspaceDialog  # 惰性导入，避免拖慢启动
    ...
    def _open_entity_admin(self) -> None:
        ...
        from ui.entity_admin__ui import EntityAdminDialog  # 惰性导入，避免拖慢启动
    ...
    def _open_ledger_admin(self, table_name: str = "contract_projects") -> None:
        ...
        from ui.ledger_admin__ui import LedgerAdminDialog  # 惰性导入，避免拖慢启动
    ...
    def _open_ledger_inbox(self) -> None:
        ...
        from ui.ledger_inbox__ui import LedgerInboxDialog  # 惰性导入，避免拖慢启动
    ...
    def _refresh_inbox_hint(self) -> None:
        ...
        try:
            import desens.ledger_inbox__desens as inbox
    ...
    def _open_graph_tools(self) -> None:
        ...
        from ui.graph_admin__ui import GraphAdminDialog
    ...
    def _build_ai_chat_dock(self) -> None:
        ...
        """
        from ui.chat_panel__ui import ChatDock  # 惰性导入，避免拖慢启动
    ...
    def _start_fix_folder(self, folder: Path) -> None:
        ...
        """
        from ui.filename_fix__ui import perform_renames, scan_folder
        from ui.fix_rename_ui__ui import FixPreviewDialog
    ...
    def _on_fix_scan_done(self, items: object) -> None:
        ...
        from ui.fix_rename_ui__ui import FixApplyWorker, FixPreviewDialog
...
def main() -> int:
    ...
    try:
        ...
        if _ws.active() is None and not _ws.list_workspaces():
            print("[仓库] 还没有登记任何仓库：本次按老的默认路径运行（hub/ + .env 的库名）。\n"
                  "        要启用隔离，先跑：python -m infra.workspace__infra register-legacy")
        else:
    ...
    try:
        from infra.monitor__infra import start_performance_monitor
    ...
    from ui.splash__ui import SplashWindow, StartupThread
