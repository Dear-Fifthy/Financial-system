...
LOG_DIR = Path(__file__).resolve().parent / "logs" / "perf"
...
def _warmup() -> None:
    ...
    这里先摸一次，再快照。
    """
    for name in ("psutil", "project_registry__desens", "self_entity__desens", "subprocess", "ctypes"):
        try:
            __import__(name)
        except Exception:
            continue
...
def _clear_app_caches() -> list[str]:
    ...
    cleared: list[str] = []
    for mod_name, fn_name in (("project_registry__desens", "invalidate_cache"),
                              ("self_entity__desens", "invalidate_cache"),
                              ("dedup__desens", None),
                              ("edge_build__graph_edges", None)):
        try:
