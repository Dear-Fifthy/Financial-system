...
命令行的自检（不用跑 24 题）：
    python -m ai.query_planner__ai --rules      # 看规则路由对一批问句的判定
    python -m ai.query_planner__ai --overview   # 看当前仓库统计块
    python -m ai.query_planner__ai --plan "库里有多少份文件？"   # 走完整规划（会调一次 graph 链）
"""
...
def _hub_dir() -> Path:
    try:
        from infra.workspace__infra import hub_root
